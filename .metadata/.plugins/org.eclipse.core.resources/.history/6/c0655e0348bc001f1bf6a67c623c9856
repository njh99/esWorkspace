...
<%@page import="co.kh.dev.makelogin.model.ZipCodeVO"%>
...
if (check.equals("n") == true) {
	...
	zipList = mdao.selectZipCode(zvo);
}
%>
<!DOCTYPE html>
<!-- ① -->
...
<body>
	<main>
		...
		<table>
			...
			<tr>
				<td><a
					href="javascript:sendAddress('<%=tempZipcode%>','<%=tempSido%>','<%=tempGugun%>','<%=tempDong%>','<%=tempBunji%>')">
						<%=data.toString()%></a><br> <%
 } //end for
 } //zipList.isEmpty()
 } //check.equals end
